...
from okex_futures_client import FuturesClient
...
contract_type = 'Okex_Swap_data'
contract_code = 'BTC-USD-SWAP'
contract_period = 60

db_name = contract_type
table_name = 'BTC_USDT_Swap_1min'

from_time = '2020-10-25 00:00:00'
# from_time = '2021-04-01 00:00:00'
to_time =   '2021-01-01 00:00:00'
frequency = 60 * 60 * 1 * 1
...
class OKex_data_crawler():
    ...
    def __init__(self):
        self.okex_spot_client = FuturesClient(api_key = OKex_api_key,
    ...
    def get_k_lines(self, from_time, to_time, contract_code = contract_code,period = contract_period):
        ...
        column_names = ('Datetime', 'high', 'open', 'low','close','vol','current_vol')
        for i in raw_data:
            columnn_values = (
                self.datetime_plus_eighthour(self.iso2datetime(i[0])),
                float(i[2]),
                float(i[1]),
                float(i[3]),
                float(i[4]),
